...
// injectAuthAndRequestBody injects the delegated token as Authorization in the
// header phase and sets ModeOverride=BUFFERED so the body arrives next for the
// Authorize check. Header mutations must happen here — in FULL_DUPLEX_STREAMED
// mode, header mutations in body-phase responses are silently dropped.
