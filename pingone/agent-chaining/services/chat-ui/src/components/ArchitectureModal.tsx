      ...
      >
        ...
        <div className="p-6 flex flex-col gap-3">
          ...
          <div className="mt-4 border border-border p-4 font-mono text-xs space-y-2">
            ...
            <TokenChainRow label="user_token" claims="sub=user  aud=support-agent  scope=order-status:invoke" color="text-emerald-400" />
            <div className="text-text-secondary pl-4">↓ RFC 8693 exchange (agent credentials as actor)</div>
            <TokenChainRow label="delegated_token" claims="sub=user  act.sub=support-agent  aud=order-status-agent  scope=order-status:invoke" color="text-sky-400" />
            <div className="text-text-secondary pl-4">↓ RFC 8693 exchange (ext_proc IDP credentials as actor)</div>
            <TokenChainRow label="tool_token" claims="sub=user  act.sub=order-status-agent  aud=order-status-mcp-server  scope=order:read" color="text-amber-400" />
          </div>
        </div>
      </div>
    </div>
...
const NODES: FlowNode[] = [
  ...
  { id: 'mcp',     label: 'Order Status MCP Server',    sub: 'Cloud Run',                 accent: false },
  { id: 'orders',  label: 'Order Status MCP Server', sub: 'Cloud Run',                accent: false },
];
